...
"""
# Apenas aliases Groq (perfis IPython que já tinham o bloco LineA antes desta extensão)
ipython_groq_aliases_append = """
# Groq (API OpenAI-compatible). Nas settings do Jupyter AI, use a chave Groq em OPENAI_API_KEY.
_default_aliases.update({
    "groq-gpt-oss-120b": "groq:openai/gpt-oss-120b",
    "groq-qwen3-32b": "groq:qwen/qwen3-32b",
    "groq-llama-3.3-70b": "groq:llama-3.3-70b-versatile",
...
if "jupyter_ai_magics" not in existing:
    ...
    if "groq-gpt-oss-120b" not in existing:
        with open(ipython_cfg_path, "a") as f:
            f.write(ipython_groq_aliases_append)
        print(f"Updated (aliases Groq): {ipython_cfg_path}")
